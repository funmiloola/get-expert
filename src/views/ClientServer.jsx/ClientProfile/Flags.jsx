  ...
  return (
    <section className="pt-8 font-sans">
      ...
      <div className="flex flex-col gap-6 pt-6">
        {reports.map(
          ({ date, comment, reportBy, status, handledBy, actions }) => (
            <div className="border border-[#C9D0D8] p-6 rounded-xl">
              <h3 className="font-medium text-sm text-[#030712] pb-4">
                {date}
              </h3>
              <p className="text-sm text-[#030712] font-bold">
                Comment: <span className="font-normal">{comment}</span>
              </p>
              <div className="flex flex-col md:flex-row gap-2 md:gap-8 py-4">
                <p className="flex flex-col justify-center text-xs font-semibold text-[#030712]">
                  Report By
                  <span className="font-bold text-base">{reportBy}</span>
                </p>
                <p className="flex flex-col justify-center text-xs font-semibold text-[#030712]">
                  Status<span className="font-bold text-base">{status}</span>
                </p>
                <p className="flex flex-col justify-center text-xs font-semibold text-[#030712]">
                  Handle By
                  <span className="font-bold text-base">{handledBy}</span>
                </p>
              </div>
              <div className="flex flex-col md:flex-row gap-2 md:gap-4.25">
                {actions.map((action) => (
                  <p
                    ...
                    } ${action === "Warn Client" ? "text-[#DC2626]" : null}`}
                  >
                    {action}
                  </p>
                ))}
